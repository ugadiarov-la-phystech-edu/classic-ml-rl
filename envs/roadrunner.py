...
class RoadrunnerEnv(gym.Env):
    ...
    def __init__(self, render_mode="none", size=10, discrete=True, negative_reward_size=-100, max_episode_steps=50):
        super().__init__()
    ...
    def draw_agent(self, position, size):
        ...
        position[0] + (2 * size / SIZE_SCALER), position[1] + size / SIZE_SCALER, size / SIZE_SCALER,
        size / SIZE_SCALER))  # eye
        pygame.draw.ellipse(self.screen, BLACK, (
        position[0] + (size) - (2 * size / SIZE_SCALER), position[1] + (size / SIZE_SCALER), size / SIZE_SCALER,
        size / SIZE_SCALER))  # eye
    ...
    def draw_sign(self, position, size):
        ...
        pygame.draw.line(self.screen, BLACK, (position[0] + (size / 2), position[1] + size),
                         (position[0] + (size / 2), position[1] + (size / 2)), 5)  # signpost
    ...
    def step(self, action):
        ...
        # First update speed with action
        ...
        # Then update location with new speed
        new_x = self._agent_location[0] + new_dx
        ...
        # An episode is done if the agent has reached the target OR has timed out
        if new_x == self._target_location[0]:
            terminated = True
            reward = 1
        elif new_x == (self._target_location[0] - 1):
            terminated = True
            reward = 0
...
gym.register(
    id=f"RoadrunnerEnv-17-v0",
    entry_point="envs.roadrunner:RoadrunnerEnv",
    kwargs={
        "size": 17,
    },
)
...
if __name__ == '__main__':
    env = gym.make('RoadrunnerEnv-17-v0', new_step_api=True)
    o = env.reset()
    done = False
    while not done:
