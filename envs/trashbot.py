...
class TrashBot(gym.Env):
    def __init__(self, container_width=100, render_mode='human',
...
class TrashBotDiscreteEnv(TrashBot):
    def __init__(self, container_width=90, n_bins=5, action_scaling=2, mode='angle', **kwargs):
        super().__init__(container_width, action_scaling=action_scaling, mode=mode)
        self.mode = mode
        self.n_bins = n_bins
        self.action_mapping = np.linspace(*self.action_range, self.n_bins + 2)[1:-1]
        self.action_space = Discrete(self.n_bins * self.n_bins)

    def _process_action(self, action):
        ## Linear interpolation from action_range to n_bins
        bins = self.action_mapping
        bin_action = np.unravel_index(action, shape=(self.n_bins, self.n_bins))
        return np.array([bins[bin_action[0]], bins[bin_action[1]]]) * self.action_scaling
...
class TrashBotConfig:
    ...
    bucket = dict(coords=[240, 290], height=80, color='g', movable=False, margin=0, is_container=True)
    colors = dict(
        w=(255, 255, 255),
        k=(0, 0, 0),
        r=(200, 100, 100),
...
class Box:
    def __init__(self, coords: tuple, width: int, height: int, color='k', margin=0, is_water=False):
        self.origin = coords
        self.width = width
        self.height = height
        self.dx, self.dy = 0, 0
    ...
    def render(self, screen: pygame.Surface):
        ...
        if self.is_container:
            pygame.draw.polygon(screen, mapping['darkgrey'],
                                [(x__, y__), (x__ + w, y__), (x__ + w - 10, y__ + h), (x__ + 10, y__ + h)], width=4)
            pygame.draw.polygon(screen, mapping['darkgrey'],
                                [(x_, y_), (x_ + w, y_), (x_ + w - 10, y_ + h), (x_ + 10, y_ + h)], width=4)
            pygame.draw.polygon(screen, color, [(x, y), (x + w, y), (x + w - 10, y + h), (x + 10, y + h)])
        else:
        ...
        if bottom > TrashBotConfig.water_level and not self.is_water:
            ...
            if self.is_container:
                pygame.draw.polygon(screen, color, [(x, uy), (x + w, uy), (x + w - 10, uy + d), (x + 10, uy + d)])
            else:
                pygame.draw.rect(screen, color, [x, uy, w, d])
        if self.is_container:
            pygame.draw.polygon(screen, mapping['darkgrey'],
                                [(x - 2, y), (x + w + 2, y), (x + w - 8, y + h), (x + 8, y + h)], width=5)
        else:
            pygame.draw.rect(screen, mapping['darkgrey'], [x, y, w, h], width=4)


class Crate(Box):
    def __init__(self, coords: tuple, width: int = 50, height: int = 50,
                 color='k', movable=True, margin=0, is_container=False):
...
def generate_cloud(surface, x, y, size):
    ...
    for i in range(size):
        ...
        ellipse_rect = pygame.Rect(x + ellipse_offset_x, y + ellipse_offset_y, ellipse_width, ellipse_height)
...
gym.register(
    id=f"TrashBotDiscreteEnv-v0",
    entry_point="envs.trashbot:TrashBotDiscreteEnv",
    kwargs={
        "num_bins": 11,
    },
)
...
if __name__ == '__main__':
    env = gym.make('TrashBotDiscreteEnv-v0')
    o = env.reset()
    done = False
    while not done:
