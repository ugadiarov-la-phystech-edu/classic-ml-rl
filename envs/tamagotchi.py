...
class TamagotchiEnv(gym.Env):
    ...
    def __init__(self, vocab = ["play", "sleep", "feed", "clean"], max_msg_length=1, tau=1.0, steps_per_episode=100, render_mode=None):
        ...
        self.observation_space = spaces.Box(low=0, high=self.n_states, shape=(1,), dtype=np.int64)
...
gym.register(
    id=f"TamagotchiEnv-v0",
    entry_point="envs.tamagotchi:TamagotchiEnv",
    kwargs={
        "max_msg_length": 1,
        "tau": 0.01,
    },
