...
class Study(gym.Env):
    metadata = {"render_modes": ["human", "graphical", None]}

    def __init__(self, total_days: int = 10, n_actions: int = 5, lectures_days: int = 3, lectures_needed: int = 2,
                 energy_needed: int = 1, seed: int = None, action_reward_noise_mean=0.5, action_reward_noise_sigma=0.05,
                 render_mode=None):
    ...
    def step(self, action: ActType) -> tuple[ObsType, SupportsFloat, bool, dict[str, Any]]:
        ...
        return np.array([self.knowledge, self.energy, self.current_day]), reward, terminated, {}
                ...
                for col in range(len(self.rendergrid[0])):
                    ...
                    if row == 0:
                        self.screen.blit(self.imgs_days[col % 5],
                                         ((col % 5) * self.cell_width + 3 * self.cell_width / 2,
                                          2 * render_row_counter * self.cell_height + 3 * self.cell_height / 8))
                        if self.rendergrid[row][col] == 1:
                            self.screen.blit(self.img_lecture,
                                             ((col % 5) * self.cell_width + 3 * self.cell_width / 2,
                                              2 * render_row_counter * self.cell_height + 6 * self.cell_height / 8))
                        ...
                        if col < self.current_day:
                            if self.rendergrid[0][col] == 1 and self.action_history[
                                col] == 2:  # studied at the right moment
                                curr_img = self.font.render('Study', True, (127, 176, 105))
                            elif self.rendergrid[0][col] != 1 and self.action_history[
                                col] == 2:  # studied at the wrong moment
                        ...
                        if col == self.current_day:
                            ...
                            pygame.draw.arc(self.screen, (0, 0, 0),
                                            pygame.Rect(mouth_pos[0] - mouth_radius, mouth_pos[1] - mouth_radius,
                                                        2 * mouth_radius, 2 * mouth_radius), 3.54, 5.88, 3)
...
gym.register(
    id=f"Study-v0",
    entry_point="envs.study:Study",
    kwargs=dict(
        total_days=10, n_actions=7, lectures_days=2, lectures_needed=1, energy_needed=1, action_reward_noise_mean=0.0,
        action_reward_noise_sigma=0.0,
    )
)


if __name__ == '__main__':
    env = gym.make('Study-v0')
    o = env.reset()
    done = False
    while not done:
