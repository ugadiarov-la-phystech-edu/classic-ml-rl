    ...
    def render(self):
        if self.render_mode == "terminal":
            ...
            def draw_door(d, x):
                ...
                # Calculate the position to blit the text on the door surface
                text_x = door_width // 2 - text_surface.get_width() // 2
                text_y = door_height // 2 - text_surface.get_height() // 2

                # Blit the text onto the door surface
            ...
            def draw_light(d, x):
                light_color_on = (243, 188, 87) #(255, 255, 0)  # Yellow color when the light is on
                ...
                pygame.draw.ellipse(self.screen, (0, 0, 0), (x - 2, light_y - 2, light_height + 4, light_width + 4))
                pygame.draw.ellipse(self.screen, light_color, (x, light_y, light_height, light_width))

                # Draw a smaller white circle inside the light to create a highlight effect
                if d == obs:
            ...
            # Fill the screen surface with the room colors
            ...
            # draw a new surface and rotate it
...
gym.register(
    id=f"MemoryCorridorEnv-v0",
    entry_point="envs.memory_corridor:MemoryCorridorEnv",
    kwargs={
        "num_doors": 3,
    },
)
...
if __name__ == '__main__':
    env = gym.make('MemoryCorridorEnv-v0')
    o = env.reset()
    done = False
    while not done:
